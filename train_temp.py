...
from utils import get_temp_model, tempDataset, tempInferDataset, compute_metrics
...
def train():
    ...
    ds = tempDataset(root, pathin, get_transform())
    ds_val = tempInferDataset(root, pathin_val, get_transform())
    ...
    for epoch in range(num_epochs):
        ...
        df_loss.to_csv('metrics'+ output_filename + '.csv', index = False)
        ...
        if val_mean_loss <= out_loss:
            ...
            torch.save(last_checkpoint, os.getcwd() + '/model' + output_filename +'.pt')
            out_loss = val_mean_loss
        torch.cuda.empty_cache()
